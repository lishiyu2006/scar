...
def generate_expression_matrix(
    ...
    """
    Generate a random expression matrix with genes as rows and cells as columns.

    Parameters:
        n_genes (int): Number of genes (rows)
        n_cells (int): Number of cells (columns)
        binary (bool): Whether to generate a 0/1 binary matrix
        sparsity (float): Sparsity rate (higher means more zeros)
        save_path (str): Optional path to save as .csv
    ...
    if binary:
        ...
        # Generate random numbers directly in float32 to avoid intermediate float64 arrays
    ...
    if save_path:
        ...
        if add_timestamp:
            # Add timestamp, format: YYYYMMDD_HHMMSS
