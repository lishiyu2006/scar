...
import logging
#import pyreadr
from scar.data_preprocessor import prepare_columns_for_loading
...
def input_with_timeout(prompt, timeout, default):
    user_input = [default]  # Use list to allow modification
    ...
    if thread.is_alive():
        print(f"\nInput timed out, using default path: {default}")
        return default
    else:
        return user_input[0]
...
def split_df_by_columns(df: pd.DataFrame, batch_size: int):
    """
    Return sub-DataFrames in batches by columns, ensuring genes as rows and cells as columns.
    """
    n_cols = df.shape[1]
...
def load_expression_matrix(path: str, batch_size: int = 2000):
    """
    Automatically detect file format and read the expression matrix in column-wise batches per cell, returning a DataFrame iterator.
...
def read_h5_by_cols(h5_path: str, batch_size: int):
    ...
    gene_names = store.select('expr', start=0, stop=1).index  # Read only gene names
...
def csv_read_by_rows(csv_path, batch_size=2000, start=1):
    ...
    if data_cols is None:
        ...
        logger.info(f"Column/index extraction complete")    
    
    if data_cols is None:
        raise ValueError("Must provide data_cols parameter to avoid re-reading column names")
...
#def read_rds_by_cols(path: str, batch_size: int = 2000):
    # Read .rds/.RData file (assumed expression matrix saved by Seurat, rows are genes, columns are cells)
...
def read_npy_by_cols(path: str, batch_size: int = 2000):
    ...
    # Default: rows are genes, columns are cells
