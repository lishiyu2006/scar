...
def compute_metrics_with_any_after_full_calc(
    ...
    logger.info(f"Filter thresholds support_th={support_th} confidence_th={confidence_th} lift_th={lift_th} leverage_th={leverage_th} conviction_th={conviction_th}")
    ...
    with open(result_dir, 'w', newline='') as f:
        ...
        logger.info("Start chunked filtering and saving results")
        for start in range(0, n_genes, chunk_size):
            end = min(start + chunk_size, n_genes)
            logger.info(f"Processing gene row block {start}:{end}")

            # Only take current block rows to save memory
            blocks = [
            ...
            # Global row & col indices for current block
            ...
            rows_idx = coords[:, 0] + start  # Global row index
            cols_idx = coords[:, 1]

            # Extract metric values from GPU (block-wise)
            ...
            logger.info(f"Number of elements meeting thresholds in current block: {len(rows_idx)}")
